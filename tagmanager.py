...
class TagManager:
    def __init__(self):
        ...
        self.file_names = {}  # dictionary to store the names of each respective img and txt file pair, as well as the
                              # image file type.
    ...
    def start(self):
        for filename in os.listdir(self.root_directory):  # checks each file in directory
            if filename.endswith(".png") or filename.endswith("jpeg") or filename.endswith("jpg"):
                ...
                self.file_names.update({name: imgtype})
    ...
    def tag_searcher(self, tag, choice=None, folder_name=None):
        ...
        if choice is None:  # searches for txt files with the specified tag by default
            choice = "with"
        ...
        count = 0
        for filename in self.file_names:  # filename is each key in the file_names dictionary
            txtfile = filename + ".txt"  # since the image and txt files have the same names
            with open(os.path.join(self.root_directory, txtfile), 'r') as file:
                contents = file.read()  # tags in each txt file
                if condition(contents):
                    imgfile = filename + "." + self.file_names[filename]
                    shutil.copy(os.path.join(self.root_directory, txtfile), os.path.join(directory2, txtfile))
                    shutil.copy(os.path.join(self.root_directory, imgfile), os.path.join(directory2, imgfile))
